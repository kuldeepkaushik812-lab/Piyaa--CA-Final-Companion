...
export const StudyTimer: React.FC<StudyTimerProps> = ({ 
      ...
      if (slot) {
        ...
        if (parsed) {
          ...
          if (currentMins >= parsed.start && currentMins < endMins) {
            const slotRemainingMins = endMins - currentMins;
            finalMins = Math.min(finalMins, slotRemainingMins);
          }
        }
  ...
  // Rehydrate active timer session on mount (across tab closes or Android app switches)
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (parsed.isRunning && parsed.endTime) {
          ...
          if (remaining > 0) {
            setWorkMinutes(parsed.workMinutes || 25);
            setBreakMinutes(parsed.breakMinutes || 5);
