...
export const TimetablePlanner: React.FC<TimetablePlannerProps> = ({
  ...
  const handleUpdateManualSlot = (id: string, updates: Partial<TimetableSlot>) => {
    setManualSlots(prev => {
      ...
      if (updates.time && index < updatedList.length - 1) {
        const parts = updates.time.split('-').map(str => str.trim());
        if (parts.length === 2) {
  ...
  const saveSlots = (newSlots: TimetableSlot[]) => {
    ...
    setScheduleForDate(selectedDateStr, newSlots);
    ...
    if (!isPastDate) {
      ...
      if (newSlotIdx !== -1 && newSlotIdx < updatedSlots.length - 1) {
        ...
        for (let i = newSlotIdx + 1; i < updatedSlots.length; i++) {
          ...
          if (s.completed || s.status === 'COMPLETED' || s.status === 'NA' || s.isFrozen) {
  ...
  const saveEdit = (slotId: string, e: React.MouseEvent) => {
    ...
    const updated = slots.map(s => s.id === slotId ? newSlot : s);
    const editIndex = slots.findIndex(s => s.id === slotId);
    
    // Auto-update ripple effect for subsequent pending slots ONLY if NOT a past date
    if (!isPastDate && editIndex !== -1 && editIndex < updated.length - 1) {
      let currentEndMin = endMin;
      for (let i = editIndex + 1; i < updated.length; i++) {
        const slot = updated[i];
        if (slot.completed || slot.status === 'COMPLETED' || slot.status === 'NA' || slot.isFrozen) {
  ...
  const handleDeleteSlot = (id: string, e?: React.MouseEvent) => {
    ...
    if (targetSlot && targetSlot.completed && targetSlot.category !== 'break') {
      const matchSubj = subjects.find(sub => 
        sub.name.toLowerCase().includes(targetSlot.subject.toLowerCase()) || 
        sub.code.toLowerCase().includes(targetSlot.subject.toLowerCase())
      );
      if (matchSubj) {
        addStudyLog(matchSubj.id, -parseSlotHours(targetSlot.time), selectedDateStr);
      }
    }
              ...
              <div className="lg:col-span-5 space-y-5 flex flex-col">
                ...
                {secondarySubject !== 'N/A' ? (
                  <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800/80 space-y-3 shadow-inner">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-extrabold text-indigo-300 uppercase tracking-wider">
                        Subject Time Split Split:
                      </label>
                    ...
                    {/* Hour display */}
                    <div className="text-center font-bold text-xs text-indigo-300 bg-indigo-950/50 py-1.5 px-3 rounded-xl border border-indigo-500/10">
