  ...
  persist(
    (set, get) => ({
          ...
          slots.forEach((slot) => {
            if (slot.category !== 'break' && slot.category !== 'na' && slot.status !== 'NA') {
              ...
              if (slot.status === 'COMPLETED' || slot.completed) {
                slotCompletedHours += totalSlotHours;
              } else if (slot.status === 'PARTIALLY_COMPLETED' || (slot.progress && slot.progress > 0)) {
                slotCompletedHours += studied;
              }
            }
          });
        }

        const logHours = (state.studyLogs || [])
          .filter((l) => l.date === dateStr)
          .reduce((acc, log) => acc + log.hours, 0);

        const historyHours = (state.studyHistoryLogs || [])
          .filter((l) => l.dateStr === dateStr)
          .reduce((acc, log) => acc + (log.durationHours || 0), 0);

        return Number(Math.max(slotCompletedHours, logHours, historyHours).toFixed(2));
      },
      ...
      splitSlotAndMorphSubject: (dateStr, slotId, newSubjectName, newTopicName) => {
        ...
        if (studiedHrs > 0) {
          get().logStudyActivity({
            ...
            chapterTitle: currentSlot.activity,
            durationHours: Number(studiedHrs.toFixed(2)),
      ...
      logStudyActivity: (entry) => {
        set((state) => {
          if (entry.sourceType === 'TIME_TABLE') {
            const isDuplicate = (state.studyHistoryLogs || []).some(
              (log) => log.sourceType === 'TIME_TABLE' && 
                       log.dateStr === entry.dateStr && 
                       log.subjectId === entry.subjectId && 
                       log.chapterTitle === entry.chapterTitle
            );
